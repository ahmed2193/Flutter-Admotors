...
mixin Utils {
  ...
  static Brightness getBrightnessForAppBar(BuildContext context) {
    if (Platform.isAndroid) {
      return isLightMode(context) ? Brightness.light :Brightness.dark;
    } else {
  ...
  static String checkUserLoginId(PsValueHolder? psValueHolder) {
    if (psValueHolder == null || psValueHolder.loginUserId == null || psValueHolder.loginUserId == '') {
      return 'nologinuser';
    } else {
  ...
  static void subscribeToModelTopics(List<String?> modelList) {
    ...
    modelList.forEach((String? model) => FirebaseMessaging.instance.subscribeToTopic(model ?? '')); 
  }

  static void unSubsribeFromModelTopics(List<String?> modelList) {
    // ignore: avoid_function_literals_in_foreach_calls
    modelList.forEach((String? model) => FirebaseMessaging.instance.unsubscribeFromTopic(model ?? ''));
  }
